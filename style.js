...
function initLGPDBanner() {
    if (!localStorage.getItem('lgpd_consent')) {
        ...
        banner.innerHTML = `
            <div>
                <strong>Nós respeitamos sua privacidade.</strong> Utilizamos cookies e tecnologias semelhantes para melhorar sua experiência em nosso site, de acordo com nossa Política de Privacidade.
            </div>
            <button id="btnAcceptCookies" class="btn btn-aa-primary btn-sm px-4 py-2">Ciente e Aceito</button>
        `;
...
function updateBusinessStatus() {
    ...
    if (currentDay >= 1 && currentDay <= 5 && currentHour >= 8 && currentHour < 18) {
        statusText = "Aberto Agora - Atendimento Comercial";
